...
if (isset($_POST['update'])) {
    ...
    $sql_matricula = "SELECT COUNT(*) AS total FROM aluno WHERE matricula = :matricula AND id <> :id";
    $stmt_matricula = $conexao->prepare($sql_matricula);
    $stmt_matricula->bindParam(':matricula', $matricula, PDO::PARAM_STR);
    $stmt_matricula->bindParam(':id', $id, PDO::PARAM_INT);
    $stmt_matricula->execute();
    $result_matricula = $stmt_matricula->fetch(PDO::FETCH_ASSOC);

    if ($result_matricula['total'] > 0) {
        echo "Matrícula já cadastrada. Por favor, insira uma matrícula única.";
        exit;
    }
